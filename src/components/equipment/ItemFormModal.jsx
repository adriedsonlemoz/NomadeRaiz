...
export function ItemFormModal({ item, defaultCat, onClose }) {
  ...
  const submit = () => {
    ...
    const payload = { ...form, price, quantity: Number(form.quantity)||1 };
    isEdit ? updateItem({ ...item, ...payload }) : addItem(payload);
    onClose();
  };
      ...
      <div onClick={e=>e.stopPropagation()} style={{ width:"100%", maxWidth:480,
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {["pendente","comprado"].map(s=>(
              <button key={s} onClick={()=>set("status",s)} style={{ flex:1, padding:"10px 0",
                borderRadius:11, fontWeight:700, fontSize:13, cursor:"pointer",
                background:form.status===s?(s==="comprado"?T.doneCheck:T.blue):T.blueChip,
                color:form.status===s?"#fff":T.textSub,
                border:`1.5px solid ${form.status===s?(s==="comprado"?T.doneCheck:T.blue):T.border}` }}>
                {s==="comprado"?"✓ Adquirido":"⏳ Pendente"}
              </button>
            ))}
          </div>
