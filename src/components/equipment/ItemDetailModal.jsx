...
export function ItemDetailModal({ item, onClose, onEdit }) {
  ...
  const handleToggle = () => {
    ...
    toggle(item.id);
    onClose(); // fechar modal após marcar
  };

  const commitPrice = () => {
    const v = parseFloat(priceInput.replace(",", "."));
    if (!isNaN(v) && v >= 0) updatePrice(item.id, v);
        ...
        <div style={{ padding:"16px 18px", display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"4px 10px",
              ...
              {{urgente:"Urgente",medio:"Médio",baixo:"Baixo"}[item.priority]}
            </span>
          </div>
