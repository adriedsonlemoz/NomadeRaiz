...
export function PontoForm({ ponto, onSave, onClose }) {
  const { theme: T } = useTheme();
  const [tipo,       setTipo]       = useState(ponto?.tipo||"agua");
  ...
  const [avaliacao,  setAvaliacao]  = useState(ponto?.avaliacao||2);
  const [fechado,    setFechado]    = useState(ponto?.fechado||false);
  const inp = { width:"100%", padding:"10px 12px", border:`1.5px solid ${T.border}`, borderRadius:10,
    ...
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:50,
      ...
      <div onClick={e=>e.stopPropagation()} style={{ width:"100%", maxWidth:480,
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
          ...
          <button onClick={()=>{ if(!nome.trim())return; onSave({tipo,nome,referencia,obs,avaliacao,fechado}); }}
