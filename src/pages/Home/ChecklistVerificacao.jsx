...
export function ChecklistVerificacao({ modoId, onVoltar }) {
  const { state, toggleCheck, resetChecks } = useStore();
  const { theme: T } = useTheme();
  const { light } = useHaptics();
  const [dicaModal, setDicaModal] = useState(null);
  ...
  const handleToggle = (id) => { light(); toggleCheck(modoId, id); };
