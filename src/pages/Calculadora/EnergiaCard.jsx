...
export function EnergiaCard({ energia, setEnergia, T }) {
  const r = calcEnergia(energia.painel, energia.horasSol, energia.bateria, energia.powerbank, energia.equip);
  const toggleEquip = id => setEnergia(e => ({...e, equip:e.equip.map(q=>q.id===id?{...q, ativo:!q.ativo}:q)}));
  const setConsumo  = (id, v) => setEnergia(e => ({...e, equip:e.equip.map(q=>q.id===id?{...q, consumoWhDia:parseNum(v)}:q)}));
  return (
