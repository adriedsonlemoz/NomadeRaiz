...
import { calcTotal, globalStats, catStats } from "../../services/equipment.service";
...
export default function EquipamentosPage() {
  ...
  // Totais globais para o rodapé da grade
  const totalGeral     = calcTotal(state.items);
  const totalAdquirido = calcTotal(state.items.filter(i => i.status === "comprado"));
  ...
  // Se há categoria ativa, mostrar sub-tela de itens
  if (activeCat) {
    return (
      <>
        {showForm && (
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", background:T.pageBg }}>
      ...
      {/* Grade de cards de categoria — responsiva: 3 colunas quando cabe, 2 quando não */}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(120px, 1fr))",
          ...
          {CATEGORIES.map(cat => {
            ...
            const hasUrgent = state.items.some(
              i => i.categoryId === cat.id && i.priority === "urgente" && i.status === "pendente"
            );
            return (
