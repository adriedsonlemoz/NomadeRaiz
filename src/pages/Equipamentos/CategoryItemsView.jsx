...
import { catStats, calcTotal, filterItems, sortItems } from "../../services/equipment.service";
import { fmt } from "../../utils/format";
import { Bar } from "../../components/common";
import { EquipmentCard } from "../../components/equipment";

export function CategoryItemsView({ catId, onBack, onAdd }) {
  ...
  // Totais do rodapé para esta categoria
  const totalCat       = calcTotal(state.items.filter(i => i.categoryId === catId));
  const totalAdquirido = calcTotal(state.items.filter(i => i.categoryId === catId && i.status === "comprado"));
  const totalPendente  = totalCat - totalAdquirido;
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", background:T.pageBg }}>
      ...
      {/* Rodapé fixo com totais da categoria */}
