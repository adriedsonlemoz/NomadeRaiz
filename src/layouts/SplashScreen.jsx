...
export default function SplashScreen({ onDone }) {
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setFase("visivel"), 50);
    const t2 = setTimeout(() => setFase("saida"),   SPLASH_CONFIG.duracao - SPLASH_CONFIG.fadeOut);
    const t3 = setTimeout(() => onDone(),            SPLASH_CONFIG.duracao);
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999,
      ...
      opacity: fase==="saida"?0:1, transition:`opacity ${SPLASH_CONFIG.fadeOut}ms ease`,
      userSelect:"none" }}>
      <div style={{ position:"relative", width:140, height:140, display:"flex",
        ...
        <div style={{ width:110, height:110, borderRadius:"50%",
          ...
          transform:`scale(${fase==="entrada"?0.7:1})`,
          transition:"transform 600ms cubic-bezier(0.34,1.56,0.64,1)" }}>
          <BicycleIcon size={60} color="#fff"/>
        </div>
      </div>
      <div style={{ textAlign:"center",
        transform:`translateY(${fase==="entrada"?16:0}px)`,
        opacity: fase==="entrada"?0:1,
        transition:"transform 600ms cubic-bezier(0.34,1.56,0.64,1), opacity 500ms ease" }}>
        <h1 style={{ color:"#fff", fontSize:32, fontWeight:900, margin:"0 0 6px",
          letterSpacing:"-0.5px", lineHeight:1 }}>{SPLASH_CONFIG.appName}</h1>
        <p style={{ color:"rgba(255,255,255,.45)", fontSize:13, fontWeight:500,
          margin:0, letterSpacing:"0.05em" }}>{SPLASH_CONFIG.appTagline}</p>
      </div>
