...
object TravelFormJson {
    ...
    fun encodePlanning(v: PlanningSession): String = JSONObject().put("schemaVersion", 1).put("draft", planning(v.draft))
        .put("lastGenerated", v.lastGenerated?.let(::planning) ?: JSONObject.NULL).toString()

    fun decodePlanning(raw: String?): PlanningSession = runCatching {
