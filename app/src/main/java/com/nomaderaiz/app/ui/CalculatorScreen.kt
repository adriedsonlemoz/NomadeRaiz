...
internal fun CalculatorScreen(equipment:List<EquipmentItem>,draft:CalculatorDraft,save:(CalculatorDraft)->Unit,back:()->Unit){
    ...
    LazyColumn(
        ...
        item{SectionCard("Viagem"){
            DraftNumber(draft,save,"days","Dias de viagem")
        }}
        item{SectionCard("Alimentação",Icons.Outlined.Restaurant){
            DraftNumber(draft,save,"foodDailyCost","Gasto por dia (R$)")
        ...
        item{SectionCard("Água",Icons.Outlined.WaterDrop){
            ...
            DraftNumber(draft,save,"waterDailyLiters","Consumo por dia (L)")
        ...
        item{SectionCard("Energia",Icons.Outlined.Bolt){
            DraftNumber(draft,save,"energyStoredWh","Energia disponível (Wh)")
            DraftNumber(draft,save,"energyDailyWh","Consumo por dia (Wh)")
...
@Composable
private fun DraftNumber(draft:CalculatorDraft,save:(CalculatorDraft)->Unit,key:String,label:String){
    NumericField(draft[key],{save(draft.withField(key,it))},label)
}
